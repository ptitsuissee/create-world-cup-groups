...
export function DrawWheel({ team, groups, onComplete, onClose, translations }: DrawWheelProps) {
  ...
  const startDraw = () => {
    ...
    // Calculate which group the wheel will land on after rotation
    ...
    const selectedIndex = Math.floor(((360 - finalRotation) / degreesPerGroup)) % groups.length;
    
    setSelectedGroupIndex(selectedIndex);
    setSelectedGroup(groups[selectedIndex]);
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-gradient-to-br from-purple-600/90 to-pink-600/90 backdrop-blur-xl rounded-3xl p-8 max-w-2xl w-full border border-white/20 shadow-2xl relative"
      ...
      >
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl mb-4">{translations.drawingTeam}</h2>
          <div className="inline-flex items-center gap-3 bg-white/20 rounded-2xl px-6 py-4 backdrop-blur-sm border border-white/30">
            {isUrl ? (
              <img src={team.flag} alt={team.name} className="w-10 h-10 object-cover rounded-lg" />
            ) : (
          ...
          <div className={`relative w-full h-full rounded-full border-8 border-yellow-400 shadow-2xl overflow-hidden ${spinning ? 'ring-4 ring-yellow-400/50 ring-offset-4 ring-offset-transparent' : ''}`}>
            ...
            >
              ...
              {groups.map((group, index) => {
                const path = createArcPath(index, groups.length);
                const color = colors[index % colors.length];
                const textPos = getTextPosition(index, groups.length);
                ...
                return (
                  <g key={`${group.id}-${index}`}>
                    ...
                    <path
                      ...
                      stroke="none"
                    />
                    {/* Segment border for visual separation */}
                    <path
                      d={path}
                      fill="none"
        ...
        <AnimatePresence mode="wait">
          ...
          ) : !spinning && selectedGroupIndex !== null && (
            <motion.div
              key="result"
              ...
              className="text-center"
            >
              <div className="text-2xl mb-2">{translations.assignedTo}</div>
              <div className="text-4xl bg-white/20 rounded-2xl px-8 py-4 inline-block backdrop-blur-sm border border-white/30">
            ...
            >
              <div className="text-center max-w-2xl" onClick={(e) => e.stopPropagation()}>
                <motion.div
                  ...
                  className="mb-6"
                >
                  <h2 className="text-3xl md:text-4xl mb-4">{translations.assignedTo}</h2>
                </motion.div>
